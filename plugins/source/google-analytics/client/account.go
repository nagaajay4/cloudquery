...
func (c *Client) initAccounts(ctx context.Context, ids []string) error {
	...
	req := c.Management.Accounts.List().Context(ctx)
	for {
		resp, err := req.Do()
		if err != nil {
			return err
...
func (c *Client) initAccountInfo(ctx context.Context, ai *AccountInfo) error {
	...
	req := c.Management.Webproperties.List(ai.Account.Id).Context(ctx)
	for {
		resp, err := req.Do()
		if err != nil {
			return err
...
func (c *Client) initProfiles(ctx context.Context, wp *WebProperty) error {
	...
	req := c.Management.Profiles.List(wp.AccountId, wp.Id).Context(ctx)
	for {
		resp, err := req.Do()
		if err != nil {
			return err
